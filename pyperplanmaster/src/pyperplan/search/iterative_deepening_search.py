...
def unique_state_representation(state, domain_spesific_representation=None):
...
class IterativeDeepeningSearchAlgorithm:
    ...
    def __init__(self):
        ...
        self.explorednodes = 0
        self.num_of_paths = 0
        self.neibors_by_level = {}
        self.neibors_set = {}
        self.max_neibors_depth = 10
    ...
    def search(self, task, maxdepth=1000000):
        ...
        path = set()
        # actual search depth
        self.neibors_by_level[0] = set()
        depth = 1
        # run until at goal or fail to explore to the given depth
        with open(file, 'a') as f: 
            f.writelines(str(task.goals))
        print(str(task.goals))
        while depth < maxdepth:
            self.neibors_by_level[depth] = set()
            #print("depth = ", depth)
            with open(file, "a") as f:
                str_print = "depth = " + str(depth) + "\n"
                f.writelines(str_print)
                print(str_print)
            self.maxreacheddepth = 0
            #self.explorednodes = 0
            plan = self.deepening_search_step(task, task.initial_state, depth, 0, path)
            ...
            # yy yifat added code
            self.neibors_set.clear()
            for level in self.neibors_by_level.keys():
                self.neibors_by_level[level].clear()
    ...
    def deepening_search_step(self, task, state, depth, step, path):
        ...
        if step < depth:
            if step + 1 == depth:
                self.num_of_paths += 1
                str_print = " num_of_paths = " + str(self.num_of_paths) + "\n"
                if self.num_of_paths % 100000 == 1:
                #    print(str_print)
                    with open(file, "a") as f:
                        f.writelines(str_print)
            nextstep = step + 1
            # remember the actual state
            path.add(unique_state_representation(state))
            self.neibors_set[unique_state_representation(state)] = step 

            self.neibors_by_level[step].add(unique_state_representation(state))
            for operator, successor_state in task.get_successor_states(state):
                self.explorednodes += 1
                # already on path? Yes then it is an loop, so ignore the
                # successor and return to the caller without a plan
                level = None
                seen_earlier = False
                if unique_state_representation(successor_state) in self.neibors_set:
                   level =  self.neibors_set[unique_state_representation(successor_state)]
                   seen_earlier = level <= nextstep
                str_print = "    " * step + str(operator) + ":" + str(level) +"?"+ str(nextstep) + "   "  + unique_state_representation(successor_state) + "\n"#"\t\t\t" + str(self.neibors_set) + "\n"
                #print(str_print)
                #with open(file, "a") as f:
                #    f.writelines(str_print)
                #if unique_state_representation(successor_state) not in path:
                if unique_state_representation(successor_state) not in path and not seen_earlier:     
                    if task.goal_reached(successor_state):
                        logging.info("Goal reached. Start extraction of " "solution.")
                        self.maxreacheddepth = nextstep
                        return [operator]
            ...
            # found no plan on the explored path and children
            # remove the actual state
            path.remove(unique_state_representation(state))
            depth_to_remove = depth + self.max_neibors_depth
            if self.neibors_by_level.get(depth_to_remove) != None:
                states_to_remove = self.neibors_by_level.pop(depth_to_remove) 
                for saved_state in states_to_remove:
                    self.neibors_set.pop(saved_state)
